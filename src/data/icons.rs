...
pub static ICONS: Lazy<Mutex<HashMap<String, PathBuf>>> =
    Lazy::new(|| Mutex::new(get_system_icons_paths()));
...
fn default_icon() {
    ...
    if !Path::new(&default_icon).exists() {
        if let Err(e) = fs::write(default_icon, DEFAULT_ICON) {
...
pub fn get_system_icons_paths() -> HashMap<String, PathBuf> {
    let mut icons = HashMap::new();
    default_icon();
    let home_dir = std::env::var("HOME").unwrap();
    let icon_theme_name = {
        if Path::new(&(home_dir.clone() + "/.config/gtk-4.0/settings.ini")).exists() {
            let settings = fs::read_to_string(home_dir + "/.config/gtk-4.0/settings.ini").unwrap();
            ...
            icon_theme.split('=').collect::<Vec<&str>>()[1]
                .trim()
                .to_string()
        } else if Path::new(&(home_dir.clone() + "/.config/gtk-3.0/settings.ini")).exists() {
            let settings = fs::read_to_string(home_dir + "/.config/gtk-3.0/settings.ini").unwrap();
            let icon_theme = settings
                .lines()
                .find(|line| line.starts_with("gtk-icon-theme-name"))
                .unwrap();
    ...
    for dir in data_dirs {
        if Path::new(&(dir.to_string() + "/icons/" + &icon_theme_name)).exists() {
...
fn find_folders_recursively(init_path: &PathBuf, folder_name: &str) -> Option<Vec<PathBuf>> {
    let mut folders: Vec<PathBuf> = vec![];
    for entry in fs::read_dir(init_path).unwrap() {
...
fn find_icons_recursively(init_path: &PathBuf) -> Option<HashMap<String, PathBuf>> {
    let mut icons = HashMap::new();
    for entry in fs::read_dir(init_path).unwrap() {
        let entry = entry.unwrap();
